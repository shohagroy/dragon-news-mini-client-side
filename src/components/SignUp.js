...
const SignUp = () => {
    ...
    const userSignUphandelar = (e)=>{
        ...
        const strongRegex = new RegExp("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])").test(password);
        if(strongRegex){
    ...
    return (
        <div className='flex justify-center items-center'>
            <div className="w-full max-w-md p-8 space-y-3 rounded-xl bg-white text-gray-900">
                ...
                <p className={`${signUpError ? 'block' : 'hidden'} text-2xl font-bold text-red-600`}>{signUpError.slice(5)}</p>

                <form onSubmit={userSignUphandelar} className="space-y-6 ng-untouched ng-pristine ng-valid">
                    <div className="space-y-1 text-sm">
                        <label  className="block text-left text-gray-400">Your Name</label>
                        <input type="text" name="name" placeholder="Your Name" className="w-full px-4 py-3 rounded-md border-gray-700 bg-gray-100 text-gray-900 focus:border-violet-400" />
                    </div>
                    <div className="space-y-1 text-sm">
                        <label className="block text-left text-gray-400">Your Email</label>
                        <input type="email" name="email"placeholder="Your email" className="w-full px-4 py-3 rounded-md border-gray-700 bg-gray-100 text-gray-900 focus:border-violet-400" />
                    </div>
                    <div className="space-y-1 text-sm">
                    ...
                    <button className="block w-full p-3 text-center rounded-sm text-white font-bold bg-blue-600">Sign Up</button>
                </form>
                ...
                <p className="text-xs text-center sm:px-6 text-gray-400">Already have an account?
                    <Link to='/login' className="underline text-gray-900">Log in</Link>
                </p>
            </div>
        </div>
    );
